...
int main(int argc, const char** argv) {
    ...
    std::vector<std::experimental::filesystem::path> queryFiles = ShapeDescriptor::utilities::listDirectory(queryImageDirectory.value());
    ...
    imageBuffer.content = new ShapeDescriptor::QUICCIDescriptor[std::max<unsigned int>(sequentialParallelSearchResults.size(), 1)];
    imageBuffer.length = std::max<unsigned int>(sequentialParallelSearchResults.size(), 1);

    ShapeDescriptor::QUICCIDescriptor blankImage;
    std::fill(blankImage.contents, blankImage.contents + ShapeDescriptor::QUICCIDescriptorLength, 0);
    std::fill(imageBuffer.content, imageBuffer.content + imageBuffer.length, blankImage);

    for(int searchResult = 0; searchResult < sequentialParallelSearchResults.size(); searchResult++) {
        imageBuffer.content[searchResult] = sequentialParallelSearchResults.at(searchResult).image;
    }
    //imageBuffer.images[0] = chosenQueryImage;

    ShapeDescriptor::dump::descriptors(imageBuffer, "searchResults_weighted_" + std::to_string(randomSeed) + ".png", 10);
